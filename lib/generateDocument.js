...
// Facility-level SOPs are Good Manufacturing Practices (21 CFR Part 117
// Subpart B prerequisite programs) — a different regulatory category from
// the core FSVP procedural SOPs (Subpart L activities like supplier
// approval and verification). Rendered as their own document section.
const GMP_KEYS = ["sanitation", "pestControl", "preOpInspection", "preventiveMaintenance", "waterTesting", "temperatureMonitoring"];
...
// and rendered in full here. Keys in GMP_KEYS above are facility-level GMPs
// and are rendered in their own section, not mixed with the core SOPs. ----------
const SOP_LIBRARY = {
  supplierApproval: {
  ...
  sanitation: {
    title: "Sanitation GMP",
    citation: "21 CFR Part 117 Subpart B",
  ...
  preOpInspection: {
    title: "Pre-Operational Inspection GMP",
    citation: "21 CFR Part 117 Subpart B",
  ...
  preventiveMaintenance: {
    title: "Preventive Maintenance GMP",
    citation: "21 CFR Part 117 Subpart B",
  ...
  waterTesting: {
    title: "Water Potability & Annual Testing GMP",
    citation: "21 CFR Part 117 Subpart B (if applicable)",
  ...
  temperatureMonitoring: {
    title: "Temperature Monitoring GMP",
    citation: "21 CFR Part 117 Subpart B (if applicable)",
...
// Renders one entry's hazard analysis results (§2.N) — hazards identified,
// the Q3/Q4 decision-tree outcomes, and the §1.502(c) subsection if
// applicable. The methodology itself (the decision tree) is explained once
// at the top of Section 2; this is just the per-entry result of applying it.
function hazardSection(e, idx) {
  e = e || {};
  const out = [];
  out.push(H2(`2.${idx + 1}  ${e.supplierName || "Unnamed Supplier"} — ${e.productName || "Unnamed Product"}`));
...
// entry in d.entries[]: who the supplier is, what the product is, and how
// the importer approves/verifies that supplier. The hazard analysis result
// that drives the verification pathway lives in Section 2, referenced here.
function entrySection(e, idx) {
  ...
  out.push(P(`See Section 2.${idx + 1} for this entry's hazard analysis and the resulting verification-pathway determination.`, { italics: true, size: 18, color: "666666" }));
...
async function generateFsvpDocument(d, selectedSops) {
  ...
  const coreSops = selectedSops.filter((k) => !GMP_KEYS.includes(k));
  const gmpSops = selectedSops.filter((k) => GMP_KEYS.includes(k));
  ...
  // Table of contents — auto-populated by Word from the Heading 1/2/3
  // styles below. Word shows a prompt to update fields on open; if it
  // doesn't, select the TOC and press F9 (or Right-click > Update Field).
  ...
  // 2. Hazard Analysis — methodology once, then each entry's results
  children.push(pageBreak());
  children.push(H1("2. Hazard Analysis"));
  children.push(P("Regulatory basis: 21 CFR 1.504. A written hazard analysis is required for each food imported, identifying known or reasonably foreseeable biological, chemical, and physical hazards, and evaluating severity and probability absent controls. The methodology below is applied individually to each foreign supplier/food product combination; results for each combination follow in Sections 2.1–2." + entries.length + "."));
  ...
  children.push(H2("Hazard Analysis Results by Supplier/Product"));
  entries.forEach((e, idx) => {
    children.push(...hazardSection(e, idx));
  });

  // 3. Supplier & Product Profiles — one subsection per entry
  children.push(pageBreak());
  children.push(H1("3. Supplier & Product Profiles"));
  children.push(P(`This FSVP plan covers ${entries.length} foreign supplier / food product combination${entries.length === 1 ? "" : "s"}, each profiled individually below. See Section 2 for each entry's hazard analysis.`));
  entries.forEach((e, idx) => {
    children.push(...entrySection(e, idx));
  });

  // 4. Standard Operating Procedures — core FSVP procedural SOPs only
  if (coreSops.length) {
    children.push(pageBreak());
    ...
    coreSops.forEach((key) => {
      const sop = SOP_LIBRARY[key];
      if (sop) children.push(...renderSop(sop));
    });
  }

  // 5. Good Manufacturing Practices — facility-level GMPs (21 CFR Part 117
  // Subpart B), only relevant if the importer also operates a domestic
  // facility. Kept separate from the FSVP procedural SOPs above since
  // they're a different regulatory category.
  if (gmpSops.length) {
    children.push(pageBreak());
    children.push(H1("5. Good Manufacturing Practices (GMPs)"));
    children.push(P("The following are Good Manufacturing Practices under 21 CFR Part 117, Subpart B, applicable because the importer also operates a domestic facility that handles the imported food. These are prerequisite programs distinct from the FSVP procedural SOPs in Section 4."));
    gmpSops.forEach((key) => {
      const sop = SOP_LIBRARY[key];
      if (sop) children.push(...renderSop(sop));
    });
  ...
  // 6. Corrective Actions (shared)
  children.push(pageBreak());
  children.push(H1("6. Corrective Actions"));
  ...
  // 7. Reassessment (shared)
  children.push(pageBreak());
  children.push(H1("7. Reassessment Schedule"));
  ...
  const doc = new Document({
    ...
    styles: {
      default: {
        document: {
          run: { font: FONT, size: 22 }, // 11pt body text, applied to any run that doesn't set its own size
        },
      },
      paragraphStyles: [
        {
          ...
          next: "Normal",
          quickFormat: true,
          run: { font: FONT, size: 32, bold: true, color: ORANGE }, // 16pt
        ...
        },
        {
          ...
          next: "Normal",
          quickFormat: true,
          run: { font: FONT, size: 26, bold: true, color: DARK }, // 13pt
        ...
        },
        {
          ...
          next: "Normal",
          quickFormat: true,
          run: { font: FONT, size: 22, bold: true, italics: true }, // 11pt
